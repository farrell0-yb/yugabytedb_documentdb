						   ...
						   pgbson *createIndexesMessage, bool *volatile snapshotSet)
{
	...
	*snapshotSet = false;
	...
	foreach(indexDefCell, createIndexesArg.indexDefList)
	{
		...
		{
			/* Option C: create the ic_ table structure synchronously (fast:
			 * CREATE TABLE + CREATE INDEX + metadata row with is_valid=false),
			 * then queue the backfill so the background worker populates ic_
			 * rows and marks both option_c_indexes.is_valid and
			 * collection_indexes.is_valid. */
			bool isUnsharded = (collection->shardKey == NULL);
			CreateOptionCSideTableStructureOnly(collectionId, indexDef, indexId,
												 isUnsharded);
			char *cmd = psprintf(
				"SELECT %s.option_c_backfill_ic(%lu::bigint, %d)",
				ApiInternalSchemaName,
