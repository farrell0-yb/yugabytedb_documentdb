...
bool IsOptionCSideTableIndex(IndexDef *indexDef);
void CreateOptionCSideTableIndex(uint64 collectionId, IndexDef *indexDef,
								  int indexId, bool useSerialExecution);
...
/* Maximum number of fields in a compound Option C index. */
#define OPTION_C_MAX_INDEX_FIELDS 4

/*
 * IsOptionCSideTableIndex returns true for any regular ascending index with
 * 1–OPTION_C_MAX_INDEX_FIELDS fields and no unsupported options.
 */
bool
IsOptionCSideTableIndex(IndexDef *indexDef)
{
	if (indexDef->unique == BoolIndexOption_True ||
		indexDef->sparse == BoolIndexOption_True ||
		indexDef->partialFilterExpr != NULL ||
	...
	int nfields = list_length(indexDef->key->keyPathList);
	if (nfields < 1 || nfields > OPTION_C_MAX_INDEX_FIELDS)
		return false;

	/* Every key path must be a regular ascending non-wildcard field. */
	ListCell *lc;
	foreach(lc, indexDef->key->keyPathList)
	{
		IndexDefKeyPath *kp = (IndexDefKeyPath *) lfirst(lc);
		if (kp->indexKind != MongoIndexKind_Regular || kp->isWildcard)
			return false;
	}

	/* Every direction must be 1 (ascending). */
	bson_iter_t keyDocIter;
	PgbsonInitIterator(indexDef->keyDocument, &keyDocIter);
	while (bson_iter_next(&keyDocIter))
	{
		if (!BSON_ITER_HOLDS_INT32(&keyDocIter) || bson_iter_int32(&keyDocIter) != 1)
			return false;
	}
...
/*
 * AppendNumericExpr appends a COALESCE expression that extracts a double
 * precision value from the jsonb expression `src`.  src may be either a
 * scalar field reference (e.g. "j -> 'price'") or an array element "elem".
 */
static void
AppendNumericExpr(StringInfo buf, const char *src)
{
	appendStringInfo(buf,
					 "COALESCE("
					 "(%s ->> '$numberDouble')::double precision,"
					 "(%s ->> '$numberInt')::double precision,"
					 "(%s ->> '$numberLong')::double precision,"
					 "(%s ->> '$numberDecimal')::double precision,"
					 "CASE WHEN jsonb_typeof(%s) = 'boolean' "
					 "     THEN CASE WHEN (%s #>> '{}') = 'true' THEN 1.0 ELSE 0.0 END END,"
					 "(%s -> '$date' ->> '$numberLong')::bigint::double precision"
					 ")",
					 src, src, src, src, src, src, src);
}

/*
 * AppendTextExpr appends a CASE expression that extracts a text value from
 * the jsonb expression `src`.
 */
static void
AppendTextExpr(StringInfo buf, const char *src)
{
	appendStringInfo(buf,
					 "CASE WHEN jsonb_typeof(%s) = 'string' THEN (%s #>> '{}')"
					 "     WHEN %s ? '$oid' THEN (%s ->> '$oid')"
					 "     ELSE NULL END",
					 src, src, src, src);
}

/*
 * OptionCBackfillIndex populates ic_<collectionId>_<indexId> from all
 * existing documents.  For each combination of scalar/array fields it emits
 * one UNION ALL arm so the single INSERT covers scalars and multikey rows.
 * Documents where no indexed field has a value are silently skipped.
 * The compound multikey restriction (at most one array field per document)
 * is enforced: arms where two or more fields are arrays are omitted.
 */
static void
OptionCBackfillIndex(uint64 collectionId, int indexId, int nfields,
					  char **fieldPaths, bool useSerialExecution)
{
	StringInfo selectSql = makeStringInfo();
	bool firstArm = true;
	bool isNull = false;

	/*
	 * Emit N+1 arms:
	 *   arm 0        — all fields scalar (no array)
	 *   arm k (1..N) — field k-1 is an array, all others scalar
	 */
	for (int arm = 0; arm <= nfields; arm++)
	{
		int arrayField = arm - 1;   /* -1 → no array field */

		if (!firstArm)
			appendStringInfoString(selectSql, " UNION ALL ");
		firstArm = false;

		/* SELECT object_id, f0_n, f0_t [, f1_n, f1_t ...] */
		appendStringInfoString(selectSql, "SELECT object_id");

		for (int fi = 0; fi < nfields; fi++)
		{
			appendStringInfoString(selectSql, ", ");

			char *src;
			if (fi == arrayField)
				src = pstrdup("elem");
			else
				src = psprintf("(j -> '%s')", fieldPaths[fi]);

			AppendNumericExpr(selectSql, src);
			appendStringInfoString(selectSql, ", ");
			AppendTextExpr(selectSql, src);
		}

		/* FROM clause */
		appendStringInfo(selectSql,
						 " FROM %s.documents_%lu,"
						 " LATERAL (SELECT (%s.bson_to_json_string(document)::text::jsonb)) AS jl(j)",
						 ApiDataSchemaName, (unsigned long) collectionId, CoreSchemaName);

		if (arrayField >= 0)
			appendStringInfo(selectSql,
							 ", LATERAL jsonb_array_elements(j -> '%s') AS elem",
							 fieldPaths[arrayField]);

		/* WHERE: enforce array/scalar role for every field */
		appendStringInfoString(selectSql, " WHERE TRUE");
		for (int fi = 0; fi < nfields; fi++)
		{
			if (fi == arrayField)
				appendStringInfo(selectSql,
								 " AND jsonb_typeof(j -> '%s') = 'array'",
								 fieldPaths[fi]);
			else
				appendStringInfo(selectSql,
								 " AND jsonb_typeof(j -> '%s') != 'array'",
		...
		/* Skip documents where every indexed field is missing/null. */
		appendStringInfoString(selectSql, " AND (");
		for (int fi = 0; fi < nfields; fi++)
		{
			if (fi > 0) appendStringInfoString(selectSql, " OR ");
			if (fi == arrayField)
				appendStringInfoString(selectSql, "TRUE");   /* array arm: always has a value */
			else
				appendStringInfo(selectSql,
								 "(j ? '%s' AND jsonb_typeof(j -> '%s') != 'null')",
								 fieldPaths[fi], fieldPaths[fi]);
		}
		appendStringInfoString(selectSql, ")");
	}

	/* Wrap in INSERT */
	StringInfo insertSql = makeStringInfo();
	...
	appendStringInfo(insertSql, ") %s", selectSql->data);

	pfree(selectSql->data);

	if (useSerialExecution)
		RunQueryWithSequentialModification(insertSql->data, SPI_OK_INSERT, &isNull);
	else
		ExtensionExecuteQueryViaSPI(insertSql->data, false, SPI_OK_INSERT, &isNull);
	Assert(isNull);
...
/*
 * CreateOptionCSideTableIndex creates a per-index table ic_<cid>_<indexId>
 * in documentdb_data with two columns per indexed field (f<N>_n double
 * precision for numeric/bool/date values, f<N>_t text for string/OID
 * values), registers the index in option_c_indexes, and backfills from any
 * existing documents.
 */
void
CreateOptionCSideTableIndex(uint64 collectionId, IndexDef *indexDef, int indexId,
							bool useSerialExecution)
{
	...
	int nfields = list_length(indexDef->key->keyPathList);
	char *fieldPaths[OPTION_C_MAX_INDEX_FIELDS];
	{
		ListCell *lc;
		int fi = 0;
		foreach(lc, indexDef->key->keyPathList)
			fieldPaths[fi++] = (char *) ((IndexDefKeyPath *) lfirst(lc))->path;
	}

	/* 1. Metadata table (created once; subsequent indexes just insert a row). */
	appendStringInfo(cmdStr,
					 "CREATE TABLE IF NOT EXISTS %s.option_c_indexes ("
	...
	if (useSerialExecution)
		RunQueryWithSequentialModification(cmdStr->data, SPI_OK_UTILITY, &isNull);
	else
		ExtensionExecuteQueryViaSPI(cmdStr->data, false, SPI_OK_UTILITY, &isNull);
	Assert(isNull);

	/* 2. Per-index table: document_id + two columns per field. */
	resetStringInfo(cmdStr);
	appendStringInfo(cmdStr,
					 "CREATE TABLE IF NOT EXISTS %s.ic_%lu_%d ("
					 "document_id %s.bson NOT NULL",
					 ApiDataSchemaName, (unsigned long) collectionId, indexId,
					 CoreSchemaName);
	for (int fi = 0; fi < nfields; fi++)
		appendStringInfo(cmdStr, ", f%d_n double precision, f%d_t text", fi, fi);
	...
	/* 3. Native compound LSM index on value columns. */
	resetStringInfo(cmdStr);
	appendStringInfo(cmdStr,
					 "CREATE INDEX IF NOT EXISTS ic_%lu_%d_idx"
					 " ON %s.ic_%lu_%d (",
					 (unsigned long) collectionId, indexId,
					 ApiDataSchemaName, (unsigned long) collectionId, indexId);
	for (int fi = 0; fi < nfields; fi++)
	{
		if (fi > 0) appendStringInfoString(cmdStr, ", ");
		appendStringInfo(cmdStr, "f%d_n ASC NULLS LAST, f%d_t ASC NULLS LAST", fi, fi);
	}
	appendStringInfoString(cmdStr, ")");
	...
	/* 4. Register the index in option_c_indexes. */
	resetStringInfo(cmdStr);
	appendStringInfo(cmdStr,
					 "INSERT INTO %s.option_c_indexes"
					 " (collection_id, index_id, index_name, field_paths, is_valid)"
					 " VALUES (%lu, %d, ",
					 ApiCatalogSchemaName,
					 (unsigned long) collectionId, indexId);
	appendStringInfo(cmdStr, "%s, ARRAY[",
					 quote_literal_cstr(indexDef->name ? indexDef->name : ""));
	for (int fi = 0; fi < nfields; fi++)
	{
		if (fi > 0) appendStringInfoString(cmdStr, ", ");
		appendStringInfo(cmdStr, "%s", quote_literal_cstr(fieldPaths[fi]));
	}
	appendStringInfo(cmdStr,
					 "], true)"
					 " ON CONFLICT (collection_id, index_id) DO UPDATE SET"
					 " index_name = EXCLUDED.index_name,"
					 " field_paths = EXCLUDED.field_paths,"
					 " is_valid = true");

	if (useSerialExecution)
		RunQueryWithSequentialModification(cmdStr->data, SPI_OK_INSERT, &isNull);
	else
		ExtensionExecuteQueryViaSPI(cmdStr->data, false, SPI_OK_INSERT, &isNull);
	Assert(isNull);

	/* 5. Backfill from existing documents. */
	OptionCBackfillIndex(collectionId, indexId, nfields, fieldPaths, useSerialExecution);

	ereport(DEBUG1, (errmsg("created Option C index ic_%lu_%d (%d field(s)): %s",
							(unsigned long) collectionId, indexId, nfields, fieldPaths[0])));
}

/*
 * CreateOptionCSideTableStructureOnly creates the ic_ table, its value index,
 * and the metadata row in option_c_indexes (with is_valid=false), but does NOT
 * populate the table.  Used by create_indexes_background so the slow backfill
 * can run in the background worker rather than blocking the caller.
 *
 * After this returns the background worker queues:
 *   SELECT documentdb_api_internal.option_c_backfill_ic(collectionId, indexId)
 * That function fills the ic_ rows and flips is_valid to true.  The worker
 * then calls MarkIndexAsValid() for the main collection_indexes entry.
 */
void
CreateOptionCSideTableStructureOnly(uint64 collectionId, IndexDef *indexDef,
									int indexId, bool useSerialExecution)
{
	StringInfo cmdStr = makeStringInfo();
	bool isNull = false;
	...
	/* 1. Metadata catalog table (created once; later indexes just insert a row). */
	appendStringInfo(cmdStr,
					 "CREATE TABLE IF NOT EXISTS %s.option_c_indexes ("
					 "collection_id bigint NOT NULL,"
					 "index_id integer NOT NULL,"
	...
	/* 3. Native compound LSM value index on the ic_ table. */
	resetStringInfo(cmdStr);
	appendStringInfo(cmdStr,
					 "CREATE INDEX IF NOT EXISTS ic_%lu_%d_idx"
					 " ON %s.ic_%lu_%d (",
	...
	/* 4. Register in option_c_indexes with is_valid=false (backfill pending).
	 * ON CONFLICT: a retry after a failed background attempt resets is_valid=false
	 * so the backfill function knows to re-run. */
	resetStringInfo(cmdStr);
	appendStringInfo(cmdStr,
					 "INSERT INTO %s.option_c_indexes"
					 " (collection_id, index_id, index_name, field_paths, is_valid)"
	...
	ereport(DEBUG1, (errmsg(
						 "created Option C ic_%lu_%d structure (%d field(s)), backfill deferred",
						 (unsigned long) collectionId, indexId, nfields)));
}
...
{
	if (IsOptionCSideTableIndex(indexDef))
	{
		CreateOptionCSideTableIndex(collectionId, indexDef, indexId, isUnsharded);
		return;
	}
