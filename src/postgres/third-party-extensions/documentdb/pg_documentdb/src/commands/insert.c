...
static void MaintainOptionCScalarIndexEntriesForInsert(uint64 collectionId,
														  pgbson *objectId,
														  pgbson *document);
							 ...
							 BatchInsertionResult *batchResult, ExprEvalState *evalState)
{
	...
	while (insertIndex < list_length(insertions))
	{
		...
		/*
		 * The Option C prototype maintains side-table entries per document.
		 * Keep the optimistic multi-insert path disabled until it performs
		 * equivalent per-document index maintenance.
		 */
		if (false && list_length(insertions) > 1 && !hasBatchedInsertFailed)
		{
				 ...
				 text *transactionId, ExprEvalState *evalState)
{
	...
	if (transactionId == NULL)
	{
		...
		if (insertResult > 0)
		{
			MaintainOptionCScalarIndexEntriesForInsert(collection->collectionId,
												  objectIdPtr, insertDoc);
		}
		pfree(paramListInfo);
			   ...
			   pgbson *document)
{
	...
	{
		MaintainOptionCScalarIndexEntriesForInsert(collectionId, objectId, document);
	}
...
/*
 * MaintainOptionCIndexEntriesForInsert maintains all Option C per-index
 * tables (ic_<cid>_<idx>) for a newly inserted document.
 */

/* Holds the extracted value for one indexed field (either numeric or text). */
typedef struct OptionCFieldValue
{
	bool	present;	/* field exists with a supported type */
	bool	isArray;	/* field value is a BSON array */
	bool	hasNumeric;
	double	numericVal;
	bool	hasText;
	char   *textVal;	/* palloc'd, valid when hasText */
} OptionCFieldValue;

/* Extract a scalar bson_value_t into an OptionCFieldValue. */
static void
OptionCExtractScalar(bson_type_t vtype, const bson_value_t *val,
					 OptionCFieldValue *out)
{
	out->present = true;
	out->isArray = false;
	out->hasNumeric = false;
	out->hasText = false;
	out->textVal = NULL;
	...
	switch (vtype)
	{
		case BSON_TYPE_DOUBLE:
			out->hasNumeric = true;
			out->numericVal = val->value.v_double;
			break;
		case BSON_TYPE_INT32:
			out->hasNumeric = true;
			out->numericVal = (double) val->value.v_int32;
			break;
		case BSON_TYPE_INT64:
			out->hasNumeric = true;
			out->numericVal = (double) val->value.v_int64;
			break;
		case BSON_TYPE_DECIMAL128:
		{
			char decStr[BSON_DECIMAL128_STRING];
			bson_decimal128_to_string(&val->value.v_decimal128, decStr);
			out->hasNumeric = true;
			out->numericVal = strtod(decStr, NULL);
			break;
		}
		case BSON_TYPE_BOOL:
			out->hasNumeric = true;
			out->numericVal = val->value.v_bool ? 1.0 : 0.0;
			break;
		case BSON_TYPE_DATE_TIME:
			out->hasNumeric = true;
			out->numericVal = (double) val->value.v_datetime;
			break;
		case BSON_TYPE_UTF8:
			out->hasText = true;
			out->textVal = pnstrdup(val->value.v_utf8.str, val->value.v_utf8.len);
			break;
		case BSON_TYPE_OID:
		{
			char oidStr[25];
			bson_oid_to_string(&val->value.v_oid, oidStr);
			out->hasText = true;
			out->textVal = pstrdup(oidStr);
			break;
		}
		default:
			out->present = false;
...
/*
 * OptionCExtractFromArrayIter extracts the current element of a bson_iter_t
 * (which must be positioned inside an array) into an OptionCFieldValue.
 */
static void
OptionCExtractFromArrayIter(bson_iter_t *iter, OptionCFieldValue *out)
{
	bson_type_t t = bson_iter_type(iter);
	const bson_value_t *v = bson_iter_value(iter);
	OptionCExtractScalar(t, v, out);
}

/*
 * One row to be inserted into ic_<cid>_<idx>.
 * Supports up to OPTION_C_MAX_INDEX_FIELDS fields.
 */
#define OPTION_C_MAX_INDEX_FIELDS 4

typedef struct OptionCIndexRow
{
	bool	hasNumeric[OPTION_C_MAX_INDEX_FIELDS];
	double	numericVal[OPTION_C_MAX_INDEX_FIELDS];
	bool	hasText[OPTION_C_MAX_INDEX_FIELDS];
	char   *textVal[OPTION_C_MAX_INDEX_FIELDS];
} OptionCIndexRow;

/* Returns true if two OptionCIndexRows have identical values for all nfields. */
static bool
OptionCRowsEqual(const OptionCIndexRow *a, const OptionCIndexRow *b, int nfields)
{
	for (int fi = 0; fi < nfields; fi++)
	{
		if (a->hasNumeric[fi] != b->hasNumeric[fi]) return false;
		if (a->hasNumeric[fi] && a->numericVal[fi] != b->numericVal[fi]) return false;
		if (a->hasText[fi] != b->hasText[fi]) return false;
		if (a->hasText[fi] && strcmp(a->textVal[fi], b->textVal[fi]) != 0) return false;
	}
	return true;
}

void __attribute__((noinline))
MaintainOptionCIndexEntriesForInsert(uint64 collectionId, pgbson *objectId,
									 pgbson *document)
{
	if (SPI_connect() != SPI_OK_CONNECT)
		ereport(ERROR, (errmsg("could not connect to SPI manager")));

	/* Bail out silently if the metadata table does not yet exist. */
	StringInfoData query;
	initStringInfo(&query);
	appendStringInfo(&query,
					 "SELECT to_regclass('%s.option_c_indexes') IS NOT NULL",
					 ApiCatalogSchemaName);
	int spiStatus = SPI_execute(query.data, true, 1);
	pfree(query.data);
	...
	bool isnull;
	bool tableExists = DatumGetBool(SPI_getbinval(SPI_tuptable->vals[0],
	...
	/* Fetch all valid indexes for this collection. */
	initStringInfo(&query);
	appendStringInfo(&query,
					 "SELECT index_id, field_paths "
					 "FROM %s.option_c_indexes "
					 "WHERE collection_id = $1 AND is_valid",
					 ApiCatalogSchemaName);
	Oid metaArgTypes[1] = { INT8OID };
	Datum metaArgValues[1] = { Int64GetDatum((int64) collectionId) };
	const char metaNulls[1] = { ' ' };
	spiStatus = SPI_execute_with_args(query.data, 1, metaArgTypes, metaArgValues,
									  metaNulls, true, 0);
	pfree(query.data);
	if (spiStatus != SPI_OK_SELECT || SPI_processed == 0)
	{
	...
	/* Copy index metadata out before the next SPI call overwrites the tuptable. */
	uint64 nIndexes = SPI_processed;
	int32  *idxIds      = (int32  *) palloc(nIndexes * sizeof(int32));
	int    *idxNfields  = (int    *) palloc(nIndexes * sizeof(int));
	char ***idxPaths    = (char ***) palloc(nIndexes * sizeof(char **));

	for (uint64 i = 0; i < nIndexes; i++)
	{
		bool isNull;
		Datum idDatum = SPI_getbinval(SPI_tuptable->vals[i],
									  SPI_tuptable->tupdesc, 1, &isNull);
		idxIds[i] = DatumGetInt32(idDatum);

		Datum pathsDatum = SPI_getbinval(SPI_tuptable->vals[i],
										  SPI_tuptable->tupdesc, 2, &isNull);
		ArrayType *arr = DatumGetArrayTypeP(pathsDatum);
		int ndims = ARR_NDIM(arr);
		int nelem = (ndims > 0) ? ARR_DIMS(arr)[0] : 0;
		idxNfields[i] = nelem;
		idxPaths[i] = (char **) palloc(nelem * sizeof(char *));
		for (int fi = 0; fi < nelem; fi++)
		{
			bool elNull;
			int subscript = fi + 1;
			Datum elDat = array_ref(arr, 1, &subscript, -1, -1, false, TYPALIGN_INT, &elNull);
			idxPaths[i][fi] = elNull ? pstrdup("") : TextDatumGetCString(elDat);
		}
	}

	/* Pre-cast objectId to bytea once; reused for every INSERT. */
	bytea *objectIdBytes = CastPgbsonToBytea(objectId);

	/* For each index, build rows and insert into ic_<collectionId>_<indexId>. */
	for (uint64 i = 0; i < nIndexes; i++)
	{
		int nfields = idxNfields[i];
		if (nfields == 0) continue;

		/*
		 * Phase 1: extract field values.  Identify which fields are arrays
		 * and enforce the MongoDB compound multikey restriction.
		 */
		OptionCFieldValue scalarVals[OPTION_C_MAX_INDEX_FIELDS];
		int arrayFieldIdx = -1;   /* index of the one allowed array field (-1 if none) */

		for (int fi = 0; fi < nfields; fi++)
		{
			bson_iter_t pathIter;
			memset(&scalarVals[fi], 0, sizeof(OptionCFieldValue));
			if (!PgbsonInitIteratorAtPath(document, idxPaths[i][fi], &pathIter))
				continue;

			const bson_value_t *fieldVal = bson_iter_value(&pathIter);
			if (fieldVal->value_type == BSON_TYPE_ARRAY)
			{
				if (arrayFieldIdx != -1)
				{
					/* Two array fields in a compound index — MongoDB error. */
					SPI_finish();
					ereport(ERROR,
							(errcode(ERRCODE_DOCUMENTDB_BADVALUE),
							 errmsg("cannot index parallel arrays in compound index: "
									"fields \"%s\" and \"%s\" are both arrays",
									idxPaths[i][arrayFieldIdx], idxPaths[i][fi])));
				}
				arrayFieldIdx = fi;
				scalarVals[fi].present = true;
				scalarVals[fi].isArray = true;
			}
			else
			{
				OptionCExtractScalar(fieldVal->value_type, fieldVal, &scalarVals[fi]);
			}
		}

		/*
		 * Phase 2: build the set of rows to insert.
		 * If no field is an array, there is exactly one row.
		 * If one field is an array, there is one row per unique array element.
		 */
		OptionCIndexRow rows[256];  /* cap: 256 unique array elements */
		int nrows = 0;

		if (arrayFieldIdx == -1)
		{
			/* All scalar — at most one row. */
			bool anyValue = false;
			for (int fi = 0; fi < nfields; fi++)
				if (scalarVals[fi].present && !scalarVals[fi].isArray &&
					(scalarVals[fi].hasNumeric || scalarVals[fi].hasText))
					anyValue = true;

			if (anyValue)
			{
				OptionCIndexRow *row = &rows[0];
				for (int fi = 0; fi < nfields; fi++)
				{
					row->hasNumeric[fi] = scalarVals[fi].hasNumeric;
					row->numericVal[fi] = scalarVals[fi].numericVal;
					row->hasText[fi]    = scalarVals[fi].hasText;
					row->textVal[fi]    = scalarVals[fi].textVal;
				}
				nrows = 1;
			}
		}
		...
		{
			/* One array field — iterate and deduplicate. */
			bson_iter_t pathIter2, arrayIter;
			if (PgbsonInitIteratorAtPath(document, idxPaths[i][arrayFieldIdx], &pathIter2) &&
				bson_iter_recurse(&pathIter2, &arrayIter))
			{
				while (bson_iter_next(&arrayIter) && nrows < 256)
				{
					OptionCFieldValue elemVal;
					OptionCExtractFromArrayIter(&arrayIter, &elemVal);
					if (!elemVal.present) continue;
					if (!elemVal.hasNumeric && !elemVal.hasText) continue;

					/* Build candidate row */
					OptionCIndexRow candidate;
					for (int fi = 0; fi < nfields; fi++)
					{
						if (fi == arrayFieldIdx)
						{
							candidate.hasNumeric[fi] = elemVal.hasNumeric;
							candidate.numericVal[fi] = elemVal.numericVal;
							candidate.hasText[fi]    = elemVal.hasText;
							candidate.textVal[fi]    = elemVal.textVal;
						}
						else
						{
							candidate.hasNumeric[fi] = scalarVals[fi].hasNumeric;
							candidate.numericVal[fi] = scalarVals[fi].numericVal;
							candidate.hasText[fi]    = scalarVals[fi].hasText;
							candidate.textVal[fi]    = scalarVals[fi].textVal;
						}
					}

					/* Dedup: skip if this combination already queued */
					bool dup = false;
					for (int r = 0; r < nrows && !dup; r++)
						dup = OptionCRowsEqual(&rows[r], &candidate, nfields);
					if (!dup)
						rows[nrows++] = candidate;
				}
			}
		}
		...
		/*
		 * Phase 3: INSERT all rows into ic_<collectionId>_<indexId>.
		 * Build the INSERT SQL once and execute once per row.
		 * $1 = document_id (bytea), then $2/$3 per field (numeric, text).
		 */
		StringInfoData insertSql;
		initStringInfo(&insertSql);
		appendStringInfo(&insertSql,
						 "INSERT INTO %s.ic_%lu_%d (document_id",
						 ApiDataSchemaName, (unsigned long) collectionId, idxIds[i]);
		for (int fi = 0; fi < nfields; fi++)
			appendStringInfo(&insertSql, ", f%d_n, f%d_t", fi, fi);
		appendStringInfo(&insertSql, ") VALUES (%s.bson_from_bytea($1)",
						 CoreSchemaName);
		for (int fi = 0; fi < nfields; fi++)
			appendStringInfo(&insertSql, ", $%d, $%d", 2 + fi * 2, 3 + fi * 2);
		appendStringInfoString(&insertSql, ")");

		int nParams = 1 + nfields * 2;
		Oid    argTypes[1 + OPTION_C_MAX_INDEX_FIELDS * 2];
		Datum  argValues[1 + OPTION_C_MAX_INDEX_FIELDS * 2];
		char   argNulls[1 + OPTION_C_MAX_INDEX_FIELDS * 2];

		argTypes[0]  = BYTEAOID;
		argValues[0] = PointerGetDatum(objectIdBytes);
		argNulls[0]  = ' ';
		for (int fi = 0; fi < nfields; fi++)
		{
			argTypes[1 + fi * 2]     = FLOAT8OID;
			argTypes[1 + fi * 2 + 1] = TEXTOID;
		}

		for (int r = 0; r < nrows; r++)
		{
			OptionCIndexRow *row = &rows[r];
			...
			for (int fi = 0; fi < nfields; fi++)
			{
				...
				if (row->hasNumeric[fi])
				{
					argValues[nIdx] = Float8GetDatum(row->numericVal[fi]);
					argNulls[nIdx]  = ' ';
				}
				else
				{
					argValues[nIdx] = (Datum) 0;
					argNulls[nIdx]  = 'n';
				}

				if (row->hasText[fi])
				{
					argValues[tIdx] = CStringGetTextDatum(row->textVal[fi]);
					argNulls[tIdx]  = ' ';
				}
				else
				{
					argValues[tIdx] = (Datum) 0;
					argNulls[tIdx]  = 'n';
				}
			}

			spiStatus = SPI_execute_with_args(insertSql.data, nParams, argTypes,
			...
			if (spiStatus != SPI_OK_INSERT)
			{
				...
				ereport(ERROR,
						(errcode(ERRCODE_DOCUMENTDB_INTERNALERROR),
						 errmsg("Option C: failed to insert into ic_%lu_%d",
								(unsigned long) collectionId, idxIds[i])));
			}
		}
		pfree(insertSql.data);
	}  /* end per-index loop */
...
/*
 * Legacy alias kept so callers in this file that haven't been renamed yet
 * can still compile.  Remove once all call sites are updated.
 */
static inline void
MaintainOptionCScalarIndexEntriesForInsert(uint64 collectionId, pgbson *objectId,
										   pgbson *document)
{
	MaintainOptionCIndexEntriesForInsert(collectionId, objectId, document);
}
