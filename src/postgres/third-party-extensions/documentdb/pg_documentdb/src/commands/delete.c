...
static void MaintainOptionCScalarIndexEntriesForDelete(uint64 collectionId,
														  pgbson *objectId);

/* Defined (non-static) in update.c; requires active SPI context */
extern void DeleteOptionCIndexEntriesForDocument(uint64 collectionId,
												   pgbson *objectId);
						   ...
						   bool hasShardKeyValueFilter, int64 shardKeyHash)
{
	...
	List *deletedObjectIds = NIL;

	if (OptionCScalarIndexTablesExist())
	{
		StringInfoData selectObjectIdsQuery;
		initStringInfo(&selectObjectIdsQuery);
		appendStringInfo(&selectObjectIdsQuery, "SELECT object_id FROM " );

		if (collection->shardTableName[0] != 0)
		{
			appendStringInfo(&selectObjectIdsQuery, " %s.%s", ApiDataSchemaName,
							 collection->shardTableName);
		}
		else
		{
			appendStringInfo(&selectObjectIdsQuery, " %s.documents_" UINT64_FORMAT,
							 ApiDataSchemaName, collectionId);
		}

		appendStringInfo(&selectObjectIdsQuery,
						 " WHERE document OPERATOR(%s.@@) $1::%s",
						 ApiCatalogSchemaName, FullBsonTypeName);

		if (hasShardKeyValueFilter)
		{
			appendStringInfo(&selectObjectIdsQuery, " AND shard_key_value = $2");
		}
		...
		int selectStatus = SPI_execute_with_args(selectObjectIdsQuery.data, argCount,
										   argTypes, argValues, argNulls, true, 0);
		pfree(selectObjectIdsQuery.data);

		if (selectStatus != SPI_OK_SELECT)
		{
			ereport(ERROR, (errcode(ERRCODE_DOCUMENTDB_INTERNALERROR),
							errmsg("failed to collect Option C scalar index entries for delete")));
		}
		...
		for (uint64 rowIndex = 0; rowIndex < SPI_processed; rowIndex++)
		{
			...
			if (!isNull)
			{
				deletedObjectIds = lappend(deletedObjectIds,
										 DatumGetPointer(SPI_datumTransfer(objectIdDatum,
															 false, -1)));
			}
		}
	}
	...
	if (rowsDeleted > 0)
	{
		ListCell *objectIdCell;
		foreach(objectIdCell, deletedObjectIds)
		{
			MaintainOptionCScalarIndexEntriesForDelete(collectionId,
											  (pgbson *) lfirst(objectIdCell));
		}
	}
				  ...
				  int64 shardKeyHash, DeleteOneResult *result)
{
	...
	if (rowsDeleted > 0)
	{
		...
		MaintainOptionCScalarIndexEntriesForDelete(collection->collectionId,
										  result->objectId);
	}
	else
	{
...
/*
 * OptionCScalarIndexTablesExist returns true when the Option C per-index
 * catalog table has been created in this database. The caller must already
 * have an active SPI context.
 */
static bool
OptionCScalarIndexTablesExist(void)
{
	...
	{
		ereport(ERROR, (errcode(ERRCODE_DOCUMENTDB_INTERNALERROR),
						errmsg("failed to check Option C index catalog table")));
	}

	bool isNull = false;
	Datum existsDatum = SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 1,
									  &isNull);

	return !isNull && DatumGetBool(existsDatum);
}


/*
 * MaintainOptionCScalarIndexEntriesForDelete removes all ic_<cid>_<idx> rows
 * for a deleted document. The caller must already have an active SPI context.
 */
static void
MaintainOptionCScalarIndexEntriesForDelete(uint64 collectionId, pgbson *objectId)
{
