...
static bool DeleteDocumentByTID(uint64 collectionId, int64 shardKeyHash,
								ItemPointer tid);
/* Defined (non-static) in insert.c */
extern void MaintainOptionCIndexEntriesForInsert(uint64 collectionId,
												   pgbson *objectId,
												   pgbson *document);

static bool OptionCScalarIndexTablesExist(void);
static void MaintainOptionCScalarIndexEntriesForUpdate(uint64 collectionId,
															 pgbson *objectId,
															 pgbson *updatedDocument);
static void DeleteOptionCScalarIndexEntriesForUpdate(uint64 collectionId,
														   pgbson *objectId);
						   ...
						   bool *hasOnlyObjectIdFilter)
{
	...
	*hasOnlyObjectIdFilter = objectIdFilter != NULL && !queryHasNonIdFilters;
	...
	/* Option C: preselect matching object_ids before the bulk UPDATE */
	List *optionCObjectIds = NIL;
	if (OptionCScalarIndexTablesExist())
	{
		StringInfoData selectQuery;
		initStringInfo(&selectQuery);
		appendStringInfo(&selectQuery,
						 "SELECT object_id FROM %s.%s"
						 " WHERE document OPERATOR(%s.@@) $1::%s",
						 ApiDataSchemaName, tableName,
						 ApiCatalogSchemaName, FullBsonTypeName);
		if (hasShardKeyValueFilter)
			appendStringInfoString(&selectQuery, " AND shard_key_value = $4");
		if (objectIdFilter != NULL)
			appendStringInfo(&selectQuery,
		...
		int selectStatus = SPI_execute_with_args(selectQuery.data, argCount,
												 argTypes, argValues, argNulls,
												 true, 0);
		pfree(selectQuery.data);

		if (selectStatus == SPI_OK_SELECT)
		{
			for (uint64 i = 0; i < SPI_processed; i++)
			{
				bool isNull = false;
				Datum objDatum = SPI_getbinval(SPI_tuptable->vals[i],
											   SPI_tuptable->tupdesc, 1, &isNull);
				if (!isNull)
					optionCObjectIds = lappend(optionCObjectIds,
											   DatumGetPointer(SPI_datumTransfer(
																	objDatum, false, -1)));
			}
		}
	}
	...
	/* Option C: update side-table entries for each matched document */
	if (optionCObjectIds != NIL && result.matchedDocs > 0)
	{
		StringInfoData rereadQuery;
		initStringInfo(&rereadQuery);
		appendStringInfo(&rereadQuery,
						 "SELECT document FROM %s.%s"
						 " WHERE object_id OPERATOR(%s.=) %s.bson_from_bytea($1)"
						 " LIMIT 1",
						 ApiDataSchemaName, tableName,
						 CoreSchemaName, CoreSchemaName);

		Oid rereadArgTypes[1] = { BYTEAOID };
		const char rereadNulls[1] = { ' ' };

		ListCell *cell;
		foreach(cell, optionCObjectIds)
		{
			pgbson *objectId = (pgbson *) lfirst(cell);
			Datum rereadArgValues[1] = {
				PointerGetDatum(CastPgbsonToBytea(objectId))
			};

			int rereadStatus = SPI_execute_with_args(rereadQuery.data, 1,
													 rereadArgTypes, rereadArgValues,
													 rereadNulls, true, 1);
			if (rereadStatus != SPI_OK_SELECT || SPI_processed == 0)
				continue;

			bool isNull = false;
			Datum docDatum = SPI_getbinval(SPI_tuptable->vals[0],
										   SPI_tuptable->tupdesc, 1, &isNull);
			if (isNull)
				continue;

			pgbson *updatedDoc = (pgbson *) DatumGetPointer(
				SPI_datumTransfer(docDatum, false, -1));

			MaintainOptionCScalarIndexEntriesForUpdate(collection->collectionId,
													   objectId, updatedDoc);
		}
		pfree(rereadQuery.data);
	}
	...
	else
	{
		...
		if (updateCandidate.updatedDocument != (Datum) 0)
		{
			...
			if (newShardKeyHash == shardKeyHash)
			{
				...
				if (result->isRowUpdated)
				{
					MaintainOptionCScalarIndexEntriesForUpdate(collection->collectionId,
						DatumGetPgBson(updateCandidate.objectId), updatedPgbson);
				}
			...
			else
			{
				...
				if (result->isRowUpdated)
				{
					DeleteOptionCScalarIndexEntriesForUpdate(collection->collectionId,
						DatumGetPgBson(updateCandidate.objectId));
				}
...
/*
 * OptionCIndexTablesExist returns true when the Option C metadata table
 * exists. Caller must already have an active SPI context.
 */
static bool
OptionCIndexTablesExist(void)
{
	...
	if (spiStatus != SPI_OK_SELECT || SPI_processed != 1)
		return false;
...
static bool
OptionCScalarIndexTablesExist(void)
{
	return OptionCIndexTablesExist();
}

/*
 * DeleteOptionCIndexEntriesForDocument removes all ic_<cid>_<idx> rows for
 * the given document across every Option C index on this collection.
 * Caller must already have an active SPI context.
 * Non-static so delete.c can call it (with its own SPI context).
 */
void
DeleteOptionCIndexEntriesForDocument(uint64 collectionId, pgbson *objectId)
{
	if (!OptionCIndexTablesExist())
		return;

	StringInfoData query;
	initStringInfo(&query);
	appendStringInfo(&query,
					 "SELECT index_id FROM %s.option_c_indexes "
					 "WHERE collection_id = $1 AND is_valid",
					 ApiCatalogSchemaName);
	Oid argTypes[1] = { INT8OID };
	Datum argValues[1] = { Int64GetDatum((int64) collectionId) };
	const char argNulls[1] = { ' ' };
	int spiStatus = SPI_execute_with_args(query.data, 1, argTypes, argValues,
										  argNulls, true, 0);
	pfree(query.data);
	if (spiStatus != SPI_OK_SELECT || SPI_processed == 0)
		return;

	uint64 nIdx = SPI_processed;
	int32 *idxIds = (int32 *) palloc(nIdx * sizeof(int32));
	for (uint64 i = 0; i < nIdx; i++)
	{
		bool isNull;
		idxIds[i] = DatumGetInt32(SPI_getbinval(SPI_tuptable->vals[i],
												  SPI_tuptable->tupdesc, 1, &isNull));
	}

	bytea *oidBytes = CastPgbsonToBytea(objectId);
	Oid dArgTypes[1] = { BYTEAOID };
	Datum dArgValues[1] = { PointerGetDatum(oidBytes) };
	const char dNulls[1] = { ' ' };

	for (uint64 i = 0; i < nIdx; i++)
	{
		StringInfoData delSql;
		initStringInfo(&delSql);
		appendStringInfo(&delSql,
						 "DELETE FROM %s.ic_%lu_%d "
						 "WHERE document_id OPERATOR(%s.=) %s.bson_from_bytea($1)",
						 ApiDataSchemaName, (unsigned long) collectionId, idxIds[i],
						 CoreSchemaName, CoreSchemaName);
		SPI_execute_with_args(delSql.data, 1, dArgTypes, dArgValues, dNulls, false, 0);
		pfree(delSql.data);
	}
}

/*
 * DeleteOptionCScalarIndexEntriesForUpdate — backward-compatible wrapper.
 * Owns its own SPI context.
 */
static void
DeleteOptionCScalarIndexEntriesForUpdate(uint64 collectionId, pgbson *objectId)
{
	if (SPI_connect() != SPI_OK_CONNECT)
		ereport(ERROR, (errmsg("could not connect to SPI manager")));
	DeleteOptionCIndexEntriesForDocument(collectionId, objectId);
	SPI_finish();
}

/*
 * MaintainOptionCScalarIndexEntriesForUpdate replaces all Option C index
 * entries for an updated document: delete stale rows, re-insert from the
 * updated document by delegating to the insert-path logic.
 */
static void
MaintainOptionCScalarIndexEntriesForUpdate(uint64 collectionId, pgbson *objectId,
											pgbson *updatedDocument)
{
	if (SPI_connect() != SPI_OK_CONNECT)
		ereport(ERROR, (errmsg("could not connect to SPI manager")));
	...
	/* Re-insert using the same logic as the insert write path. */
	MaintainOptionCIndexEntriesForInsert(collectionId, objectId, updatedDocument);
}
